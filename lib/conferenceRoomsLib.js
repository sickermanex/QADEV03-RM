...
var associateRoomAnother = function(roomId, resource, token){
	//var endpointAssig = resourcesEndpoint.replace("[roomId]", roomId)
	return request
		.post(resourcesEndpoint.replace("[roomId]", roomId).concat("?type=bulk"))
		.set('Authorization', token)
		.send(resource);

};

exports.associateRoomAnother = associateRoomAnother;